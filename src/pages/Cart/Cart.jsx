      ...
      <div className="container">
              ...
              <div className="cart__card-info">
                <div className="cart__sizes">
                  <div className="cart__sizes-top">
                    ...
                    <p className="cart__sizes-top__price">{item.price * item.count} ₽</p>
                  </div>
                  <div style={{ width: '200px' }} className="card__size-count">
                    <button
                      style={{ background: 'red', height: '30px', width: '50px', margin: '10px' }}
                      type="button"
                    ...
                    <button
                      style={{ background: 'green', height: '30px', width: '50px', margin: '10px' }}
                      type="button"
                  ...
                  <div className="cart__sizes-info">
                    <p className="cart__sizes-info__text">
                      Количество: <span className="cart__sizes-info__subtitle">{item.count}</span>
                    </p>
                    <p className="cart__sizes-info__text">
                      Размер(Ш×Д×В):{' '}
                      <span className="cart__sizes-info__subtitle">
                        {item.width} x {item.deep} x {item.height}
                      </span>
                    </p>
                  </div>
                </div>
              </div>
              <button onClick={() => delCarts(item.id)} className="cart__card-btn">
                <span>
                  <svg
                    width="10"
                    height="10"
        ...
        {user.carts?.length ? (
          <div className="cart__total">
            ...
            <button className="cart__total-btn" onClick={() => navigate('/checkout')}>
              Оформить заказ
            </button>
          </div>
        ) : (
